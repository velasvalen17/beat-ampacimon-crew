#!/usr/bin/env python3
"""
Show proposed roster schedule in a table format - one player per row, one column per game day.
"""
...
# Proposed roster (after 2 transactions)
PROPOSED_ROSTER = {
    'backcourt': [
...
def get_player_games_for_week():
    """Get all games for each player in the proposed roster for gameweek 9."""
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    
    # Get all players in roster
    all_players = PROPOSED_ROSTER['backcourt'] + PROPOSED_ROSTER['frontcourt']
...
def main():
    ...
    print(f"\n{'🔵 BACKCOURT':^120}")
    print("-"*120)
    for player in PROPOSED_ROSTER['backcourt']:
        print(f"{player:<25}", end="")
    ...
    print(f"\n{'🔴 FRONTCOURT':^120}")
    print("-"*120)
    for player in PROPOSED_ROSTER['frontcourt']:
        print(f"{player:<25}", end="")
        games = player_games.get(player, {})
